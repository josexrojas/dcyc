...
abstract class Paisbase extends CI_Model {
	...
	public function isNew() {
		...
		if (is_numeric($this->get('id'))) {
			$query = $this->db->get_where('paises', array('id' => $this->get('id')));
	...
	public function save()
	{
		try {
			if ($this->isNew()) {
				// insert
				if (!$this->db->insert('paises', $this->data))
					return false;
				...
				return $this->db->update('paises', $this->data);
			exit;
			}
			return true;
		} catch (Exception $ex) {
