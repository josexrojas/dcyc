...
class CI_Image_lib {
	...
	function image_process_gd($action = 'resize')
	{
		...
		else if ($action == 'resize_and_crop')
		{
			$nwidth = $this->width;
			$nheight = $this->orig_height *  $this->width / $this->orig_width;
			/*print $this->orig_width ."-".$this->orig_height . "<br>";
			print $this->width ."-".$this->height . "<br>";
			print $nwidth ."-".$nheight . "<br>";
			exit;*/
			if ($nheight - $this->height < $nwidth - $this->width)
			{
				$this->y_axis = (($nheight - $this->height) / 2) * ($this->orig_width / $this->width);
				$this->orig_height = $nheight - $this->y_axis;
			}
			else
			{
				$nheight2 = $this->height;
				$nwidth2 = $this->orig_width *  $this->height / $this->orig_height;
				
				$this->x_axis = (($nwidth2 - $this->width) / 2) * ($this->orig_height/ $this->height);
				$this->orig_width = $nwidth2 - $this->x_axis;
			}
		}
		else
		{
